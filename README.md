...
사진과 기본 지도는 로컬 파일입니다. 기본 화면은 Esri World Street Map 상세 지도를 인터넷으로 불러오며, 지명은 영어 표기를 우선으로 표시하고 도시·도로·건물을 확대해서 볼 수 있습니다. 연결이 없으면 기본 지도(오프라인)로 전환할 수 있습니다. API 키·로그인·DB가 필요 없습니다. 기존 vinext 빌드/실행 구조를 사용합니다. 호스팅 설정이나 배포는 수행하지 않았습니다.
