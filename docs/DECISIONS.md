...
User requested readable details when zooming. Use client-loaded Leaflet 1.9.4 and HTTPS Esri World Street Map raster tiles, which prioritize English map labels, with visible linked attribution and normal browser caching. No prefetch, bulk download, tile proxy, API keys or server added. See https://www.esri.com/en-us/legal/terms/full-master-agreement and https://leafletjs.com/reference.html . Keep the local Natural Earth map as a manual offline fallback. This changes the default map from offline to network-dependent; photo assets remain local. Public map tiles are suitable for this local MVP; review tile-provider capacity/policy before a wider release. Sites configuration and deployment remain untouched.
