...
export function OfflineMap({ value, answer, onChange, copy }: {
  ...
  useEffect(() => {
    ...
    function wheel(event: WheelEvent) {
      ...
      setViewport(current => {
        const box = mapWindow(current.x, current.y, current.zoom);
        const zoom = Math.max(1, Math.min(32, current.zoom * Math.exp(-Math.max(-100, Math.min(100, event.deltaY)) * 0.005)));
        const next = mapWindow(box.x + box.width / 2 - 360 / zoom, box.y + box.height / 2 - 180 / zoom, zoom);
        return { x: next.x, y: next.y, zoom };
      });
    }
  ...
  return <div className="geo-map-wrap">
    ...
    <svg ref={surface} className="geo-map" viewBox={`${box.x} ${box.y} ${box.width} ${box.height}`} preserveAspectRatio="none"
